...
public class AllCardPanel : MonoBehaviour
{
    ...
    public void MoveIn()
    {
        ...
        transform.DOMove(new Vector2(transform.position.x, transform.position.y - 800), 1f).From().OnComplete(
            () =>
            {
    ...
    public void MoveOut()
    {                
        UIManager.instance.chooseCardPanel.LockAllCardUse();
        transform.DOMove(new Vector2(transform.position.x, transform.position.y - 500), 1f).OnComplete(
            () =>
            {
